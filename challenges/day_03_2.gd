class_name AOC2024Day3Part2 extends CanvasLayer
...
func _on_results_button_pressed() -> void:
	...
	regex.compile("mul\\((\\d{1,3}),(\\d{1,3})\\)|do\\(\\)|don't\\(\\)")
	...
	for m in matches:
		#print(m.strings)
		# Casting to an Array[String] because match doesn't work with RegExMatch's strings,
		# which is a PackedStringArray.
		var array: Array[String]
		array.assign(m.strings)
		...
		match array:
			...
			_:
				if enabled:
					total += mul(int(m.strings[1]), int(m.strings[2]))
