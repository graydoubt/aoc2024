class_name AOC2024Day8Part1 extends CanvasLayer
...
class NodeTypes:
	...
	func _add_antinodes_for(node1: Vector2i, node2: Vector2i):
		var diff: Vector2i = node2 - node1
		var antinode1: Vector2i = node1 - diff
		var antinode2: Vector2i = node2 + diff
		if bounds.has_point(antinode1):
			add_node("#", antinode1)
		if bounds.has_point(antinode2):
			add_node("#", antinode2)
